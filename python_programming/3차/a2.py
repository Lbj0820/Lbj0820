...
def func_a(current_grade, last_grade, rank, max_diff_grade):
    ...
    for i in range(arr_length):
        if current_grade[i] >= 80 and rank[i] <= arr_length // 10: #점수가 80점 이상이고 등수가 배열 길이보다 작은 사람이 있으면 카운트를 증가
            count += 1
        elif current_grade[i] >= 80 and rank[i] == 1: # "" 거나 1등인 사람
            count += 1
        elif max_diff_grade > 0 and max_diff_grade == current_grade[i] - last_grade[i]: #점수차 등수가 0보다 크고 그 등수가 점수차랑 같은 경우
            count += 1
    return count

def func_b(current_grade): #이번학기중 받은 점수를 기준으로 높은 학생으로 부터 순위를 반환
    arr_length = len(current_grade)
    rank = [1] * arr_length
    for i in range(arr_length):
...
def func_c(current_grade, last_grade): #이전학기와 요번학기점수차를 구하여 차가 가장 큰 사람 1명의 점수를 반환
    max_diff_grade = 1
